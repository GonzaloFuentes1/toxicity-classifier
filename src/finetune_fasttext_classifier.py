...
def load_data(json_path: str, threshold: float, label: str, balance_method: str = None) -> None:
    """
    ...
    for x in data:
        ...
        category = next((cat for cat in x["moderation_result"]["moderationCategories"] if cat["name"] == label), None)
    ...
    if balance_method is not None:
        ...
        if balance_method == "undersample":
            ...
            if len(class_0) == 0 or len(class_1) == 0:
                print("⚠ No se puede realizar undersampling porque alguna clase no tiene muestras.")
            else:
            ...
            # Si alguna clase no tiene muestras, se mantiene el set original
            if len(class_0) == 0 or len(class_1) == 0:
                print("⚠ No se puede realizar oversampling porque alguna clase no tiene muestras.")
            else:
            ...
            print(f"⚠ Estrategia de balanceo '{balance_method}' desconocida. Se procederá sin balanceo.")
...
def evaluate_model(model: fasttext.FastText, test_file: str, model_name: str):
    ...
    Args:
        ...
        model_name (str): Nombre (o ruta) del modelo, se utiliza para nombrar el archivo de métricas.
    """
    ...
    labels_pred = [1 if model.predict(txt)[0][0] == "__label__1" else 0 for txt in test_df["text"]]

    accuracy = accuracy_score(labels_true, labels_pred)
    precision, recall, f1, _ = precision_recall_fscore_support(labels_true, labels_pred, average="binary")
...
def main():
    parser = argparse.ArgumentParser(description="FastText toxicity classifier trainer with optional autotune.")
    ...
    parser.add_argument("--word_ngrams", type=int, help="Set n-grams manually (optional)")
    parser.add_argument("--epoch", type=int, help="Set epochs manually (optional)")
    parser.add_argument("--lr", type=float, help="Set learning rate manually (optional)")
    ...
    balance_method = "undersample" if args.undersample else "oversample" if args.oversample else None
    ...
    # Si el usuario fija algún hiperparámetro, fastText lo respetará incluso con autotune
