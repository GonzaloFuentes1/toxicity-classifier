    ...
    for start_idx in range(0, total_examples, chunk_size):
        ...
        try:
            results = chunk.map(
                ...
                num_proc=1,  # Procesa en CPU; ajustable según recursos disponibles
            )
        except Exception as e:
            raise RuntimeError(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--text_column", required=True, help="Nombre de la columna de texto del dataset"
    )
    parser.add_argument(
        "--batch_size", required=True, type=int, help="Tamaño del batch"
    )
    parser.add_argument(
        "--chunk_size", required=True, type=int, help="Número de ejemplos por fragmento"
    )
    args = parser.parse_args()

    # Imprimir hiperparámetros del modelo para verificación
    print_model_hyperparameters(args.model_path)
    # Ejecutar la clasificación y procesamiento del dataset
    main(args)
