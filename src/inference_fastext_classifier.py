...
def init_model(model_path: str):
    ...
    if toxicity_classifier is None:
        try:
            ...
            raise RuntimeError(f"Error al cargar el modelo fastText desde '{model_path}': {e}")
...
def classify_toxicity(batch: dict, text_column: str) -> dict:
    ...
    Clasifica un batch de textos de la siguiente forma:
      ...
      - Si alguno de los párrafos se clasifica como '__label__1', se etiqueta
        el texto completo como '__label__1' y se asigna como score el máximo score observado.
      - Si ningún párrafo es '__label__1', se asigna '__label__0' y se toma el máximo score de cualquier párrafo.
    
    Args:
        batch (dict): Diccionario con listas de ejemplos; se espera que contenga la clave 'text_column'.
    ...
    for text in texts:
        ...
        if not paragraphs:
            # En caso de que no se encuentren párrafos válidos, asignamos label 0 y score 0.
        ...
        if is_toxic:
            ...
            toxic_scores = [pred[1][0] for pred in paragraph_preds if pred[0][0] == "__label__1"]
...
def print_model_hyperparameters(model_path: str):
    ...
    try:
        ...
        raise RuntimeError(f"Error al cargar el modelo para imprimir hiperparámetros: {e}")
...
def main(args):
    ...
    for start_idx in range(0, total_examples, chunk_size):
        ...
        try:
            ...
            raise RuntimeError(f"Error al seleccionar fragmento {start_idx}-{end_idx}: {e}")
        ...
        try:
            results = chunk.map(
                ...
                num_proc=1,  # Procesa en CPU; se puede ajustar según los recursos disponibles.
            )
        except Exception as e:
            raise RuntimeError(f"Error al procesar el fragmento {start_idx}-{end_idx}: {e}")
    ...
    try:
        ...
        raise RuntimeError(f"Error al guardar el dataset etiquetado en '{output_path}': {e}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_path", required=True, help="Ruta al modelo fastText (.bin)")
    parser.add_argument("--dataset_path", required=True, help="Ruta al dataset por etiquetar.")
    parser.add_argument("--output_path", required=True, help="Ruta para guardar el dataset etiquetado.")
    parser.add_argument("--text_column", required=True, help="Nombre de la columna de texto del dataset.")
    parser.add_argument("--batch_size", required=True, type=int, help="Tamaño del batch.")
    parser.add_argument("--chunk_size", required=True, type=int, help="Número de ejemplos por fragmento.")
    args = parser.parse_args()

    # Imprimir hiperparámetros del modelo para verificación
    print_model_hyperparameters(args.model_path)
    
    # Ejecutar la clasificación y procesamiento del dataset usando los argumentos del parser.
    main(args)
